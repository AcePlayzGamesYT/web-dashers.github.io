class PlayerState {
  ...
  reset() {
    ...
    this.isMini = false;
    this.miniGravityScale = 0.82;
    this.miniJumpBoost = 1.18;
...
// ===== MINI PHYSICS (GLOBAL SAFE FIX) =====
if (this.p.isMini && !this.p.isDead) {
  // make gravity lighter (fix heavy feel)
  if (!this.p.onGround) {
    this.p.yVelocity *= 0.985;
  }

  // boost upward movement (wider jumps)
  if (this.p.yVelocity < 0) {
    this.p.yVelocity *= 1.02;
  }
}
